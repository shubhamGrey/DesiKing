...
namespace Agronexis.DataAccess.ConfigurationsRepository
{
    public class ConfigurationRepository : IConfigurationRepository
    {
        ...
        public async Task<byte[]> GenerateInvoicePdf(GenerateInvoiceRequestModel invoiceData, string xCorrelationId)
        {
            ...
            // Step 2: Try DinkToPdf first, fallback to PuppeteerSharp for production
            try
            {
                // Generate the HTML for the invoice
                var htmlContent = GenerateInvoiceHtml(invoiceData);

                // Step 3: Configure DinkToPdf settings
                ...
                // Step 4: Use the registered IConverter (from Program.cs)
                using var scope = _serviceProvider.CreateScope();
                var converter = scope.ServiceProvider.GetRequiredService<IConverter>();

                var pdfBytes = converter.Convert(pdfDoc);
            ...
            catch (Exception dinkToPdfEx)
            {
                ...
                // Fallback to PuppeteerSharp for production environments
        ...
        private async Task<byte[]> GenerateInvoicePdfWithPuppeteer(GenerateInvoiceRequestModel invoiceData, string xCorrelationId)
        {
            try
            {
                _logger.LogInformation("Using PuppeteerSharp fallback for invoice generation, xCorrelationId: {CorrelationId}", xCorrelationId);
                ...
                try
                {
                    ...
                    _logger.LogInformation("Invoice PDF generated successfully using PuppeteerSharp, xCorrelationId: {CorrelationId}", xCorrelationId);
                    return pdfBytes;
                }
                finally
                {
                    ...
                    _logger.LogInformation("Chromium browser closed after invoice generation, xCorrelationId: {CorrelationId}", xCorrelationId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "PuppeteerSharp fallback also failed for invoice generation, xCorrelationId: {CorrelationId}", xCorrelationId);
                throw new Exception($"Both DinkToPdf and PuppeteerSharp failed to generate invoice PDF: {ex.Message}", ex);
            }
        }
